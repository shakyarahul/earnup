                ...
                <div class="card">
                    <div class="card-header">
                        <ul class="nav nav-tabs card-header-tabs">
                            <li class="nav-item"><a class="nav-link" onclick="openTab(event,'tabone')" href="#">User Info</a></li>
                            <li class="nav-item"><a class="nav-link" onclick="openTab(event,'tabtwo')" href="#">More Info</a></li>
                            <li class="nav-item"><a class="nav-link" onclick="openTab(event,'tabthree')" href="#">Experience</a></li>
                        </ul>
                    </div>
                    <div id="tabone" class="card-body tab" style="display:block">
                        <img class="w-25 h-25 img-fluid rounded mx-auto d-block" src="/storage/cover_images/{{auth()->user()->profilePic}}" onclick="document.getElementById('profilePic').click()">
                        
                        <div class="card-body">
                        <h4 class="card-title">{{auth()->user()->name}} ({{auth()->user()->status}})</h4><label> Thank you for last {{ (int)((strtotime("now") - strtotime(auth()->user()->created_at))/(60*60*24)) }} days of your services</label>
                            <blockquote class="blockquote">
                                <?php 
                                    $blockquote = explode("~" , auth()->user()->blockquote);
                                ?>
                            <p class="mb-0" contenteditable="true" onblur="updateBlockquote()" id="blockquote">{{$blockquote[0]}}</p>
                            <footer class="blockquote-footer"  >
                                <p contenteditable="true" onblur="updateBlockquote()" id="blockquotefooter">{{$blockquote[1]}}</p>
                            </footer>
                            </blockquote>
                        </div>        
                    </div>
                    ...
                    <div id="tabtwo" class="card-body tab" style="display:none">
                            ...
                            {!! Form::open(['id'=>'editprofileform','action'=>'UsersController@store','enctype'=>'multipart/form-data','method'=>'POST']) !!}
                                ...
                                <input type="hidden" value="~" name="blockquote" id="blockquoteinput">
                                    <div class="input-group-prepend"><span class="input-group-text">Full Name</span></div><input name="fname" class="form-control" type="text" value="{{auth()->user()->fname}}" placeholder="First and middle name"><input value="{{auth()->user()->lname}}" class="form-control" type="text" name="lname" placeholder="Last name">
                                    <div class="inpu t-group-append"></div>
                                </div>
                                <div class="input-group">
                                    <div class="input-group-prepend"><span class="input-group-text">Email address</span></div><input name="email" class="form-control" type="text" value="{{auth()->user()->email}}" readonly>
                                ...
                                <div class="input-group">
                                    <div class="input-group-prepend"><span class="input-group-text">Contact</span></div><input name="mobile" class="form-control" type="text" placeholder="Mobile number" value="{{auth()->user()->mobile}}"><input class="form-control" type="text" value="{{auth()->user()->phone}}" placeholder="Optional number" name="phone">
                                    <div class="input-group-append"></div>
                                </div>
                                <div class="input-group">
                                    <div class="input-group-prepend"><span class="input-group-text">Location<br></span></div><input class="form-control" type="text" id="location" readonly name="location" value="{{auth()->user()->location}}">
                                ...
                                <?php
                                    $latlng = explode("~",auth()->user()->location);
                                    $lat = $latlng[0];
                                    $lng = $latlng[1];
                                ?>
                                <script>
